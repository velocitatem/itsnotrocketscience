...
def rotation_matrix(rx=0., ry=0., rz=0.):
    ...
    # RZ * RY * RX
    # Replace np.mat with np.asmatrix for NumPy 2.0 compatibility
    Rz_matrix = np.asmatrix(Rz)
    Ry_matrix = np.asmatrix(Ry)
    Rx_matrix = np.asmatrix(Rx)
    RotationMatrix = Rz_matrix * Ry_matrix * Rx_matrix
...
def create_pose_matrix(tx=0., ty=0., tz=0.,
    ...
    TranslationMatrix = np.asmatrix(TranslationMatrix)
    RotationMatrix = np.asmatrix(RotationMatrix)
    ScaleMatrix = np.asmatrix(ScaleMatrix)
    base_correction = np.asmatrix(base_correction)
